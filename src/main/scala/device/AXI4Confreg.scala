...
class AXI4Confreg extends AXI4SlaveModule(new AXI4Lite, _extra = new UARTIO) with HasConfregConst
{
  ...
  // uart sim, do not support input sim
  io.extra.get.out.valid := (in.w.fire && waddr(15, 0) === VIRTUAL_UART_ADDR.U)
  ...
  val mapping = Map(
    ...
    RegMap(VIRTUAL_UART_ADDR, virtual_uart_data),
  )
